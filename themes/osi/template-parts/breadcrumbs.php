<?php if ( ! is_front_page() ) :
	...
	if ( ! ( $is_post_type_hierarchical || $is_taxonomy_hierarchical || is_author() ) || is_front_page() ) {
		return;
	}
	...
	<div class="breadcrumb-area">
	  <div class="wrapper">
		...
		if ( $is_post_type_hierarchical ) {
			...
			$breadcrumb .= '<span class="current-category" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><meta property="position" content="' . esc_attr( $position ) . '"><span itemprop="name">Articles by: ' . esc_html( $author_data->display_name ) . '</span></span>';
		...
		if ( is_archive() ) {
			...
			echo '<nav class="entry-breadcrumbs" itemscope itemtype="https://schema.org/BreadcrumbList" data-chris="jones">' . $home . $posts_page . $breadcrumb . '</nav>'; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		} else {
